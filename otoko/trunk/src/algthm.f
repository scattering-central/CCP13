...
C
      ...
      IMPLICIT NONE
C
C Purpose: Calculate natural or base10 anti-log of spectrum.
C
C
      INCLUDE 'COMMON.FOR'
C
...
C Local variables:
C
      REAL    VALUE(10),EXPMIN,EXPMAX,FLMIN,FLMAX
...
C
               IF (IBASE.EQ.1) THEN
                  DO 30 K=1,NCHAN
                     IF (SP1(K).LT.EXPMIN) SP1(K)=EXPMIN
                     IF (SP1(K).GT.EXPMAX) SP1(K)=EXPMAX
                     SP2(K)=EXP(SP1(K))
30                CONTINUE
               ELSE
                  DO 40 K=1,NCHAN
