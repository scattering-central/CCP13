...
C
      ...
      IMPLICIT NONE
C
C Purpose: Weighted division of two spectra, channel per channel.
C
      INCLUDE 'COMMON.FOR'
C
C Calls   7: APLOT  , ASKYES , DAWRT  , ERRMSG , GETHDR , GETVAL
C            OPNFIL
C Called by: OTOKO
C
C-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-
...
C
      ...
      INTEGER IHFMX1,IFRMX1,IHFMX2,IFRMX2,NVAL,NCHAN2
...
20          WRITE (IPRINT,1010)
         ...
         DO 30 K=1,NCHAN
            TEMP=SP2(K)*WEIGHT(2)
            IF (ABS(TEMP).LT.0.0001) THEN
               SP3(K)=0.0
            ELSE
               SP3(K)=SP1(K)*WEIGHT(1)/TEMP
            ENDIF
30       CONTINUE
C
