...
C
      ...
      IMPLICIT NONE
C
C Purpose: Linear least squares fit to a straight line
C
      INCLUDE 'COMMON.FOR'
C
C Calls   6: APLOT  , DAWRT  , GETHDR , LINFIT , OPNFIL , GETVAL
C Called by: OTOKO
C
C-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-
C Local variables:
C
      REAL    VALUE(10),SIGMAA,SIGMAB,A,B,R
      ...
      INTEGER IHFMX1,IFRMX1,IHFMX2,IFRMX2,NVAL,NCHAN2,MODE
      INTEGER NFRAM1,NFRAM2,NCHAN1,NCHAN3,I,K
...
25          MODE=0
            ...
            IF (IRC.EQ.1) GOTO 50
            IF (IRC.EQ.2) GOTO 25
            IF (NVAL.GE.1) MODE=INT(VALUE(1))
         ENDIF
         IF (MODE.EQ.-1) THEN
            DO 26 K=1,NCHAN
               SP3(K)=SQRT(ABS(SP2(K)))
26          CONTINUE
         ENDIF
         CALL LINFIT (SP1,SP2,SP3,NCHAN,MODE,A,SIGMAA,B,SIGMAB,R)
         IF (IFRAME.EQ.1) WRITE (IPRINT,1000) A,SIGMAA,B,SIGMAB,R
         DO 30 K=1,NCHAN
            SP4(K)=B*SP1(K)+A
30       CONTINUE
C
         IF (IFRAME.EQ.1) CALL APLOT (ITERM,IPRINT,SP4,NCHAN)
...
999   RETURN
C
1000  FORMAT (' Y Intercept is ',G13.5,' Standard deviation ',G13.5,/,
     1        ' Gradient is    ',G13.5,' Standard deviation ',G13.5,/,
     1        ' Linear correlation coefficient is ',G13.5)
1010  FORMAT (' Enter weighting mode:',/,
     1        ' [+1] Instrumental 1/SIGMA**2',/,
     1        ' [0]  No weighting',/,
     1        ' [-1] Statistical 1/Y   [0]: ',$)
      END
