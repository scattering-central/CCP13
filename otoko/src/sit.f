...
C
      ...
      IMPLICIT NONE
C
C Purpose: Reverse Singleton fourier transform
C
      INCLUDE 'COMMON.FOR'
C
C Calls   8: APLOT  , DAWRT  , ERRMSG , FILL   , GETCHN
C            GETHDR , GETXAX , OPNFIL , OUTFIL , SFT1D
C Called by: OTOKO
C
C-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-
C Local variables:
C
      ...
      INTEGER IHFMX1,IFRMX1,IHFMX2,IFRMX2,NCHAN1
...
C
         ...
         DO 20 J=ICH1,ICH2
            SP3(J)=SP1(J)*COS(SP2(J))
            SP4(J)=SP1(J)*SIN(SP2(J))
20       CONTINUE
C
         DO 1 K=2,NCHAN,2
            SP3(K)=-SP3(K)
