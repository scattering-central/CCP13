...
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C
      SUBROUTINE BGWLSI(B,BB,IWID,JWID,ISEP,JSEP,SMOO,TENS)
      IMPLICIT NONE
C
...
C          speed things up (inclined plane + spots).
C
...
C 
                           PRF(IW,1) = SCALEB
                           PRF(IW,2) = SCALEB*(IN-I)
                           PRF(IW,3) = SCALEB*(JN-J)
                           PIX(IW) = AD(MM)
C     
...
C 
 ...
 180  CONTINUE 
      RETURN 
C
 1000 FORMAT(/1X,'BGWLSI: Window background fitting...')
 1010 FORMAT(1X,'BGWLSI: Error - window size too large for array')
 1020 FORMAT(1X,'BGWLSI: Warning - too many peaks, resetting')
