...
C
      ...
      REAL BUF(NBUF),XPOS(MAXDIM)
      DOUBLE PRECISION A(MAXPAR),ASTDER(MAXPAR)
      LOGICAL REPEAT,FIT,INIT,XAXIS
C
C Local variables:
C
     ...
     &        IXMAXF,NLSQ,TYPE,M,KPAR,NV1,NV2,ITMAX,ISTEP,ITMP
      ...
      REAL VAL(10),SIG(MAXDIM),XFIT(MAXDIM),YFIT(MAXDIM)
...
C
 ...
 7       CONTINUE
      ...
      ENDIF
c++++++++GHOST++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 8    CALL MAP(XMIN,XMAP,YMIN,YMAX)
      CALL WINDOW(XMIN,XMAX,YMIN,YMAX)
      CALL DEFPEN
      CALL FULL
      CALL SCALES
      CALL LINCOL(3)
      CALL PTJOIN(XPOS,BUF,IXMIN,IXMAX,1)
      CALL PICNOW
      CALL DEFPEN
c++++++++PGPLOT+++++++++++++++++++++++++++++++++++++++++++++++++++++++++
c 8    CALL PGBBUF
...
c      CALL PGSCI(1)
c      CALL PGSLS(1)
c      CALL PGBOX('BCNST',0.0,0,'BCNSTV',0.0,0)
c      CALL PGSCI(3)
c      CALL PGLINE(IXMAX-IXMIN+1,XPOS(IXMIN),BUF(IXMIN))
c      CALL PGSCI(1)
c      CALL PGEBUF
c++++++++END++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      IF(.NOT.RETRY.AND..NOT.REPEAT)THEN
...
C
         IF(.NOT.REPEAT.AND..NOT.RETRY)THEN
            ...
            IF(REPLY)THEN
               ...
               CALL FLUSH(6)
c++++++++GHOST++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
               CALL CURSOR(XCURSR,YCURSR,KCHAR)
               CALL POSITN(XCURSR,YMIN)
               CALL JOIN(XCURSR,YMAX)
c++++++++PGPLOT+++++++++++++++++++++++++++++++++++++++++++++++++++++++++
c               CALL PGBBUF
c               CALL PGCURS(XCURSR,YCURSR,CH)
c               KCHAR=ICHAR(CH)
c               CALL PGMOVE(XCURSR,YMIN)
c               CALL PGDRAW(XCURSR,YMAX)
c++++++++END++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
               XMINF = XCURSR
c++++++++GHOST++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
               CALL CURSOR(XCURSR,YCURSR,KCHAR)
...
c++++++++GHOST++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            CALL LINCOL(2)
 20         CALL CURSOR(XCURSR,YCURSR,KCHAR)
            IF(KCHAR.EQ.108.OR.KCHAR.EQ.114)THEN
               IF(KCHAR.EQ.108)THEN
c++++++++PGPLOT+++++++++++++++++++++++++++++++++++++++++++++++++++++++++
c            CALL PGSCI(2)
c 20         CALL PGCURS(XCURSR,YCURSR,CH)
c            KCHAR=ICHAR(CH)
c            IF(KCHAR.EQ.65.OR.KCHAR.EQ.88)THEN
c               IF(KCHAR.EQ.65)THEN
c++++++++END++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
                  NPEAKS = NPEAKS + 1
c++++++++GHOST++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
                  CALL FULL
...
c++++++++END++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
                  ...
                  A(MA+2) = DBLE(XCURSR)
                  TEMP = (YCURSR-YMIN)*(XMAP-XMIN)/(YMAX-YMIN)
                  A(MA+3) = DBLE(0.5*ABS(TEMP))
...
C
 ...
 33      CONTINUE
     ...
     &              X1LIM(KORIG),X2LIM(KORIG))
         ...
         CALL FLUSH(6)
         IF(.NOT.REPEAT)THEN
 34         WRITE(6,'(1X,A3,1X,A7,$)')'200','SETUP: '
            CALL FLUSH(6)
            CALL RDCOMF(5,WRD,NW,VAL,NV,ITEM,10,10,10,IRC)
            IF(IRC.EQ.2)THEN
               IFLAG = -2
...
C
         ...
         ELSEIF(WRD(1)(1:3).EQ.'INI')THEN
            STEP = .FALSE.
            IF(NV.GT.0)THEN
               IF(NV.EQ.2)A(VTABLE(NINT(VAL(1)))) = DBLE(VAL(2))
            ENDIF
            IF(NW.GT.1)THEN
               CALL UPPER(WRD(2),10)
               IF(WRD(2)(1:3).EQ.'ORI')THEN
...
c++++++++GHOST++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            ...
            CALL SCALES
            CALL LINCOL(3)
            CALL PTJOIN(XPOS,BUF,IXMINF,IXMAXF,1)
c++++++++PGPLOT+++++++++++++++++++++++++++++++++++++++++++++++++++++++++
c            CALL PGBBUF
c            CALL PGPAGE
c            CALL PGSCI(1)
...
c            CALL PGBOX('BCNST',0.0,0,'BCNSTV',0.0,0)
c            CALL PGSCI(3)
c            CALL PGLINE(IXMAXF-IXMINF+1,XPOS(IXMINF),BUF(IXMINF))
c            CALL PGEBUF
c++++++++END++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
C
 ...
 50      CONTINUE
         WRITE(6,1060)I-1
         CALL FLUSH(6)
 55      IF(CONVGD.OR.I.EQ.ITMAX+1)THEN
            ENDLAM = 0.0
            CALL MRQMIN(XFIT,YFIT,SIG,NLSQ,A,MA,LISTA,MFIT,COVAR,ALPHA,
     &           MAXPAR,CHISQ,FUNCS,ENDLAM) 
            IF(NLSQ.GT.MFIT)THEN
...
c++++++++GHOST++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
         CALL ERASE
         CALL MAP(XMINF,XMAPF,YMIN,YMAX)
         CALL WINDOW(XMINF,XMAXF,YMIN,YMAX)
         CALL DEFPEN
         CALL FULL
         CALL SCALES
c++++++++PGPLOT+++++++++++++++++++++++++++++++++++++++++++++++++++++++++
c         CALL PGBBUF
c         CALL PGPAGE
c         CALL PGSWIN(XMINF,XMAXF,YMIN,YMAX)
c         CALL PGSCI(1)
c         CALL PGSLS(1)
c         CALL PGBOX('BCNST',0.0,0,'BCNSTV',0.0,0)
c         CALL PGEBUF
...
c++++++++GHOST++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
         CALL LINCOL(3)
         CALL PTPLOT(XPOS,BUF,IXMINF,IXMAXF,MARKER)
         CALL PICNOW
c++++++++PGPLOT+++++++++++++++++++++++++++++++++++++++++++++++++++++++++
c         CALL PGBBUF
c         CALL PGSCI(3)
c         CALL PGPT(IXMAXF-IXMINF+1,XPOS(IXMINF),BUF(IXMINF),MARKER)
c         CALL PGEBUF
c++++++++END++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C
C========Plot background
...
C
         RETRY = .FALSE.
         IF(.NOT.REPEAT)CALL ASK('706 Try again',RETRY,0)
         IF(RETRY)THEN
c++++++++GHOST++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            CALL ERASE
c++++++++PGPLOT+++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
C
 ...
 2010 FORMAT(1X,'***Numeric input expected')
...
c++++++++END++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      ENDIF
c++++++++GHOST++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      CALL CURVEO(XPOS,YFIT,IXMINF,IXMAXF)
      CALL PICNOW
c++++++++PGPLOT+++++++++++++++++++++++++++++++++++++++++++++++++++++++++
c      CALL PGLINE(IXMAXF-IXMINF+1,XPOS(IXMINF),YFIT(IXMINF))
c      CALL PGEBUF
c++++++++END++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      RETURN
      END
