C     LAST UPDATE 13/04/93
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C
      SUBROUTINE ROTINT
      IMPLICIT   NONE
C
C PURPOSE: Rotate an image with interpolation.
C
      INCLUDE 'COMMON.FOR'
C
...
C
      ...
      REAL         THETA,ANGLE,SINRAD,COSRAD,X,Y,DX,DY
...
C
               ...
               DO 70 K=1,NRAST
                  ...
                  DO 60 L=1,NPIX
                     ...
                     IX=INT(X+XCENTR)+IXOFF
                     IY=INT(Y+YCENTR)+IYOFF
                     IF (IX.GT.0.AND.IX.LT.NPIX.AND.
     &                   IY.GT.0.AND.IY.LT.NRAST)THEN
                         M=(IY-1)*NPIX + IX
                         DX = X + XCENTR - REAL(INT(X+XCENTR))
                         DY = Y + YCENTR - REAL(INT(Y+YCENTR))
                         SP2(N+L) = (1.-DX)*(1.-DY)*SP1(M)
     &                            +      DX*(1.-DY)*SP1(M+1)
     &                            +      (1.-DX)*DY*SP1(M+NPIX)
     &                            +           DX*DY*SP1(M+NPIX+1) 
                     ENDIF
