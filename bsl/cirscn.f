...
C
      ...
      IMPLICIT   NONE
C
C PURPOSE: Perform circular scan on an image.
C
      INCLUDE 'COMMON.FOR'
C
...
C LOCAL VARIABLES:
C
      REAL         VALUE(10),PI,XCENTR,YCENTR,RAD,COSRAD,SINRAD
      REAL         THETA,ANGLE,X,Y,DX,DY,WIDTH
...
C
               IF (KPIC.EQ.0.AND.(FIXED)) THEN
                  ...
                  IF(IRAD2.GT.IRAD1)THEN
                     RAD = REAL (IRAD1)
                     WIDTH = REAL (IRAD2-IRAD1)
                  ELSEIF(IRAD1.GT.IRAD2)THEN
                     RAD = REAL (IRAD2)
                     WIDTH = REAL (IRAD1-IRAD2)
                  ELSE
                     RAD = REAL (IRAD1)
                     WIDTH = 1.0
                  ENDIF
                  XCENTR = REAL (IXC1)
                  YCENTR = REAL (IYC1)
                  WRITE(IPRINT,1030)IXC1,IYC1,INT(RAD),INT(WIDTH)
               ENDIF
C
...
C
               ...
               KPIC=KPIC+1
C
               NAPTS=INT(2.0*PI*RAD)
               CALL FILL (SP2,NAPTS*NOUT,0.0)
               ANGLE=2.0*PI/FLOAT(NAPTS)
cx               call timer ()
               DO 70 K=1,NAPTS
