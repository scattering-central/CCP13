...
C
      ...
      IMPLICIT NONE
C
C PURPOSE: CALCULATE THE INVERSE 2-D FOURIER TRANSFORM.
C
      INCLUDE 'COMMON.FOR'
C
C Calls   8: WFRAME , GETHDR , FFT2D  , OUTFIL
...
C
      REAL         WORK(2305),TEMP
      INTEGER      NPIX1,NPIX2,NRAST1,NRAST2,NFRAM1,NFRAM2,I,K
      INTEGER      NPIX,NRAST,IRC,IMEM,KPIC,IFRAME,IDR(1024)
      INTEGER      ISPEC1,LSPEC1,MEM1,IFFR1,ILFR1,INCR1,IFINC1
      INTEGER      ISPEC2,LSPEC2,MEM2,IFFR2,ILFR2,INCR2,IFINC2
      INTEGER      IHFMX1,IFRMX1,IHFMX2,IFRMX2
...
C
C========FIND REAL & IMAGINARY PARTS AND INVERSE TRANSFORM
C
         DO 20 K=1,NPIX*NRAST
            TEMP=SP1(K)
            SP1(K)=SP1(K)*COS (SP2(K))
            SP2(K)=TEMP*SIN (SP2(K))
20       CONTINUE
