...
static int	_bslFileSelect_readHeader( swidget UxThis, Environment * pEnv, char *file, int mem, int *npixel, int *nraster, int *nframes );
...
#ifdef UX_USE_VALUES
	UxPUT_PROPERTY(bslFileSelect,x,int,577);
#else
	UxPUT_PROPERTY(bslFileSelect,x,int,490);
#endif /* UX_USE_VALUES*/
#ifdef UX_USE_VALUES
	UxPUT_PROPERTY(bslFileSelect,y,int,485);
#else
