...
{ 
    if (busyState)
    {
        ...
        if (frameD != NULL && XtIsRealized (frameD))
            XDefineCursor (UxDisplay, XtWindow (frameD), watch);
    ...
    else
    {
        ...
        if (frameD != NULL && XtIsRealized (frameD))
            XDefineCursor (UxDisplay, XtWindow (frameD), None);
  ...
  *cptr = '\0';
  ...
  if (strcmp (string, "Enter") == 0)
      ...
      if (strcmp (cptr, "output") == 0)
	{
      ...
      if (strcmp (cptr, "first") == 0)
	{
          if(header1==NULL)header1=(char*)strdup("");
          command ("%s\n", header1);
          free (header1);
          header1=NULL;
	}
	...
	{
          if(header2==NULL)header2=(char*)strdup("");
          command ("%s\n", header2);
          free (header2);
          header2=NULL;
          return (1);
      ...
      if (icode > 0 && icode != lastcode)
	{
	  switch (lastcode)
	    ...
	    case 300: 	
	      break;
	    case 400:
              if(XtIsRealized(working))UxPopdownInterface(working);
              SetBusyPointer(0);
	      break;
	    case 500:
	      break;
      ...
      else
	{
	  switch (lastcode)
	      ...
	      if ((irc = put_file (fileName, cptr)) < 0)
		{
	      ...
	      break;
	    case 208:
	      if((irc=put_backfile(sBackInFile,binfram,cptr))<0)
	      {
                sBackInFile = NULL;
              }
	      break;
	    case 209:
	      if((irc=get_backfile(sBackOutFile,cptr))<0)
	      {
                sBackOutFile = NULL;
              }
	      break;
...
{
  NewFile(sBackOutFile,npix,nrast,1,2,1,1,endian(),0);
}
