      FUNCTION PRAXIS(T0,MACHEP,H0,N,PRIN,X,F,FMIN,nfmax,iflag)
C                             LAST MODIFIED 3/1/73
C     Modified for use in lsqint 27/03/98 R.Denny
      integer idim
      parameter(idim=50)
...
C              2.22D-16) FOR REAL ARITHMETIC ON THE IBM 360.
...
c     nfmax    is the maximum number of function evaluations.
...
C
      ...
      REAL RANDOM,SQRT,ABS
C
C.....IF N>idim OR IF N<idim AND YOU NEED MORE SPACE, CHANGE 'idim' TO 
C     THE LARGEST VALUE OF N IN THE NEXT CARD, AND IN THE DIMENSION
C     STATEMENTS IN SUBROUTINES MINFIT,PMIN,FLIN,QUAD.
C
      REAL D(idim),Y(idim),Z(idim),Q0(idim),Q1(idim),V(idim,idim)
...
C     FX MUST BE PASSED TO PMIN BY VALUE.
      VALUE=FX
      CALL PMIN(N,1,2,D(1),S,VALUE,.FALSE.,F,X,T,MACHEP,H)
      if(n.eq.1)then
         iflag = 1
         go to 400
      endif
      IF (S.GT.0.0) GO TO 50
           DO 45 I=1,N
45              V(I,1)=-V(I,1)
50    IF (SF.GT.0.90*D(1).AND.0.90*SF.LT.D(1)) GO TO 70
           DO 60 I=2,N
60              D(I)=0.0
C
...
C
           IF(.NOT.ILLC) GO TO 95
                DO 90 I=1,N
                     S=(0.10*LDT+T2*(10**KT))*(RANDOM(N)-0.50)
                     Z(I)=S
                     DO 85 J=1,N
85                        X(J)=X(J)+S*V(J,I)
90              CONTINUE
...
95         DO 105 K2=K,N
                ...
                S=0.0
                VALUE=FX
                CALL PMIN(N,K2,2,D(K2),S,VALUE,.FALSE.,F,X,T,MACHEP,H)
                IF (ILLC) GO TO 97
                     S=SL-FX
                     GO TO 99
97              S=D(K2)*((S+Z(K2))**2)
...
C
           VALUE=F1
           CALL PMIN(N,K,4,D(K),LDS,VALUE,.TRUE.,F,X,T,MACHEP,H)
...
C
      CALL SPSORT(IDIM,N,D,V)
      DMIN=D(N)
      IF (DMIN.LT.SMALL) DMIN=SMALL
      ILLC=.FALSE.
      IF (M2*D(1).GT.DMIN) ILLC=.TRUE.
      IF (PRIN.GT.1.AND.SCBD.GT.1.0) CALL VCPRNT(2,Z,N)
...
200   Q(1) = AB(1,1)
      ...
      RETURN
      END
      SUBROUTINE PMIN(N,J,NITS,D2,X1,F1,FK,F,X,T,MACHEP,H)
      IMPLICIT REAL (A-H,O-Z)
      integer idim
      parameter(idim=50)
      EXTERNAL F
      ...
      COMMON /GLOBAL/ FX,LDT,DMIN,NF,NL
     .       /Q/ V,Q0,Q1,QA,QB,QC,QD0,QD1,QF1
C...THE SUBROUTINE PMIN MINIMIZES F FROM X IN THE DIRECTION V(*,J) UNLESS
...
1        QD1 = QD1 + (S-L)**2
      ...
      VALUE=QF1
      CALL PMIN(N,0,2,S,L,VALUE,.TRUE.,F,X,T,MACHEP,H)
