...
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C
      SUBROUTINE DRAGON(CELL,CSTAR,PHIZ,PHIX,ISGP,DMAX,DELR,DELZ,NLAT)
      IMPLICIT NONE
...
C
     ...
     &     TMX,V,WAV
      REAL WAV2,Z,ZER
      INTEGER I,IC,ICST,ID,IDFTMX,IEX,IH,II,IK,IL,IND,IOR,IOUT,IOW,IS,
     &        ISO,ISPHE,ITMP,J,K
...
C     Channel for reflection list.
      IOUT = 20
      WRITE(DFILE,'(A10,I1)')'DRAGON.OUT',NLAT
      OPEN(UNIT=IOUT,FILE=DFILE,STATUS='UNKNOWN')
      RADS = PI/180.0
      RADS2 = RADS/2.0
...
C     been sorted).
         WRITE(IOUT,5450)CTYPE,AR,BR,CR,ALR,BER,GAR
         NUN = 1
         MFD = MUL(IND(1))
         DO 788 I = 2,N
...
C     File output
 ...
 788     CONTINUE
         MULFD(IND(N)) = MFD
         DO 790 I = 1,N
            WRITE(IOUT,5460)IH(IND(I)),IK(IND(I)),IL(IND(I)),MUL(IND(I))
     &                      ,MULFD(IND(I)),D(IND(I)),R(IND(I)),Z(IND(I))
 790     CONTINUE
C     Write to LPT file.
         WRITE(LPT,5470)NUN
         WRITE(IOW,5470)NUN
...
C     hopefully this should never occur !
 880  WRITE(IOW,5690)
      WRITE(LPT,5690)
      CALL EXIT(0)
 5010 FORMAT(/1X,'Cell constants (a b c alpha beta gamma) '/1X,3F9.4,1X,
     &       3F8.3)
 5020 FORMAT(/1X,'Volume of unit cell is ',F10.1)
 5030 FORMAT(/1X,'Reciprocal lattice parameters (a* b* c* alpha* beta*',
     &       1X,'gamma*)'/1X,3F8.5,3F7.2)
C      WRITE(LPT,31) DMIN
C   31 FORMAT(/1X,'Minimum d-spacing is ',F8.5,' A')
C
...
C      READ(IOR,33,END=494) ISGP
 5040 FORMAT(20A1)
 5050 FORMAT(/1X,'Spacegroup ',20A1)
 5060 FORMAT(/1X,'Lattice ',A1,' 1st symbol ',6A1,' 2nd symbol ',6A1,1X,
     &       '3rd symbol ',6A1)
 5070 FORMAT(/1X,'Alternative cell constants for rhombohedral',1X,
     &       'systems'/1X,3F9.4,1X,3F8.3)
 5080 FORMAT(/1X,'There are no extinctions')
 5090 FORMAT(/1X,'Extinction rules')
 5100 FORMAT(3X,'hkl : k + l = 2n')
 5110 FORMAT(3X,'hkl : h + l = 2n')
 5120 FORMAT(3X,'hkl : h + k = 2n')
 5130 FORMAT(3X,'hkl : h + k + l = 2n')
 5140 FORMAT(3X,'hkl : - h + k + l = 3n')
 5150 FORMAT(3X,'h00 : h = 2n')
 5160 FORMAT(3X,'0k0 : k = 2n')
 5170 FORMAT(3X,'00l : l = 2n')
 5180 FORMAT(3X,'hk0 : h = 2n')
 5190 FORMAT(3X,'0kl : k = 2n')
 5200 FORMAT(3X,'h0l : l = 2n')
 5210 FORMAT(3X,'h0l : h = 2n')
 5220 FORMAT(3X,'hk0 : k = 2n')
 5230 FORMAT(3X,'0kl : l = 2n')
 5240 FORMAT(3X,'hk0 : h + k = 2n')
 5250 FORMAT(3X,'h0l : h + l = 2n')
 5260 FORMAT(3X,'0kl : k + l = 2n')
 5270 FORMAT(3X,'hk0 : h + k = 4n')
 5280 FORMAT(3X,'h0l : h + l = 4n')
 5290 FORMAT(3X,'0kl : k + l = 4n')
 5300 FORMAT(3X,'00l : l = 4n')
 5310 FORMAT(3X,'hhl : l = 2n')
 5320 FORMAT(3X,'hhl : h = 2n')
 5330 FORMAT(3X,'hh0 : h = 2n')
 5340 FORMAT(3X,'hhl : h + l = 2n')
 5350 FORMAT(3X,'hhl : 2h + l = 4n')
 5360 FORMAT(3X,'00l : l = 3n')
 5370 FORMAT(3X,'00l : l = 6n')
 5380 FORMAT(3X,'hkk : h = 2n')
 5390 FORMAT(3X,'h00 : h = 4n')
 5400 FORMAT(3X,'2h-hl : l = 2n')
 5410 FORMAT(3X,'hkk : h + 2k = 4n')
 5420 FORMAT(/1X,'Maximum indices are ',I3,' H ',I3,' K ',I3,' L ')
 5430 FORMAT(/1X,'Total number of reflections is ',I5)
 5440 FORMAT(/1X,'Sum of the multiplicities is ',I6)
 5450 FORMAT(A4,6X,6E10.4)
 5460 FORMAT(1X,3I4,2I5,1X,3E14.6)
 5470 FORMAT(/1X,'Number of unique reflections is ',I5)
C     TTY output.
C      WRITE(IOW,425)
...
C     +  D(IND(I)),R(IND(I)),TH(IND(I))
 ...
 5580 FORMAT(/1X,'Lattice calculation completed'/)
 5590 FORMAT(/1X,'Preceeding set of parameters ignored')
 5600 FORMAT(/1X,'Error - impossible cell constants')
 5610 FORMAT(/1X,'Error - "',A1,'" found at start of spacegroup',1X,
     &       'symbol ',20A1/)
 5620 FORMAT(/1X,'Error - "',A1,'" found after "',A1,'" in spacegroup',
     &       1X,'symbol ',20A1/)
 5630 FORMAT(/1X,'Error - unacceptable spacegroup'/)
 5640 FORMAT(/1X,'Error - cell constants incompatible with spacegroup'/)
 5650 FORMAT(/1X,'Error - can not find unique axis for cell'/)
 5660 FORMAT(/1X,'Error - point group incompatible with lattice type'/)
 5670 FORMAT(/1X,'Warning - core limit reached : array size set at ',I5)
 5680 FORMAT(/1X,'Generation of HKL''s stopped at',3I4)
 5690 FORMAT(/1X,'Error - internal program fault'/)
