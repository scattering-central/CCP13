...
void timer  (void);
void rframe (int *, int *, int *, int *, void *, int *);
...
void rframe (int *fd, int *frame, int *npix, int *nrast, void *buff, int *irc)
{
		  ...
		  int *npix, int *nrast, int *nframe)
{
	...
	else
	{
	    do {
	       if ((fgets (buff, MAXLIN, fp)) == NULL)
		   return (FALSE);
	       else if (sscanf (buff, "%8d%8d%8d%8d%8d", npix, nrast, nframe, &fendian, &dtype) != 5)
		   return (FALSE);
