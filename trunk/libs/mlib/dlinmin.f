...
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C
      SUBROUTINE DLINMIN(P,XI,N,FRET,FUNC,DFUNC)
      IMPLICIT NONE
C
...
C          is accomplished by calling the routines MNBRAK2 and DBRENT2. 
C
C Calls   2: NMBRAK2, DBRENT2
C Called by:
C
C-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-
C Parameters:
...
C
      REAL F1DIM,DF1DIM,DBRENT2
      EXTERNAL F1DIM,DF1DIM,DBRENT2
C
C Common block:
C
      INTEGER NCOM
...
C-----------------------------------------------------------------------
 ...
 10   CONTINUE
      ...
      FRET = DBRENT2(AX,XX,BX,FUNC,DFUNC,TOL,XMIN)
      DO 20 J=1,N
         XI(J) = XMIN*XI(J)
         P(J) = P(J) + XI(J)
 20   CONTINUE 
